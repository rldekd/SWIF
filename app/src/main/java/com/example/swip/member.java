...
public class member extends AppCompatActivity {
    ...
    public void mem_login(View view){
        ...
        if(!Pattern.matches(e_mailPattern , id1)){
            Toast.makeText(member.this , "이메일 형식을 확인해주세요." , Toast.LENGTH_LONG).show();
            return;
        }
        ...
        if(pw1.equals("") || pw2.equals("")){
            Toast.makeText(member.this , "비밀번호를 입력해주세요." , Toast.LENGTH_LONG).show();
            return;
        }
                    ...
                    .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                        ...
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            if (task.isSuccessful()) {
                                Toast.makeText(member.this, "회원가입을 성공했습니다.", Toast.LENGTH_SHORT).show();
                                FirebaseUser user = mAuth.getCurrentUser();
                                finish();
                            } else {
                                Toast.makeText(member.this, "회원가입을 실패하였습니다.", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
        }else{
